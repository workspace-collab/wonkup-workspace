...
with sync_playwright() as p:
    ...
    assert page.locator('.canvas-build-version').inner_text()=='Motor 5.9.0'
    ...
    page.fill('[data-draft-note] .canvas-inline-note-input','Nota rápida 5.9')
    ...
    quick_note=page.locator('.canvas-note[data-note-id]').filter(has_text='Nota rápida 5.9')
    quick_note.hover()
    ...
    # Detalle de nota con control compacto igual a Color de marca.
    quick_note=page.locator(f'.canvas-note[data-note-id="{note_id}"]')
    quick_note.locator('[data-open-note]').click()
    page.wait_for_selector('#wonkup-modal #note-detail-color')
    assert page.locator('#wonkup-modal .note-detail-color-palette').count()==0
    assert page.locator('#wonkup-modal #note-detail-color').get_attribute('type')=='color'
    page.locator('#wonkup-modal #note-detail-color').evaluate("el=>{el.value='#123456';el.dispatchEvent(new Event('input',{bubbles:true}))}")
    assert page.locator('#wonkup-modal #note-detail-color-code').inner_text()=='#123456'
    page.locator('#wonkup-modal #note-detail-form button[type="submit"]').click()
    page.wait_for_selector('#wonkup-modal',state='detached')
    quick_note=page.locator(f'.canvas-note[data-note-id="{note_id}"]')
    style=quick_note.get_attribute('style') or ''
    assert '--note-bg:#123456' in style.replace(' ','')
    print('DETAIL_COLOR_COMPACT_OK')
    ...
    page.screenshot(path=str(root/'tests'/'canvas-ux-smoke-5-9.png'),full_page=True)
    browser.close()
print('CANVAS_UX_5_9_OK')
