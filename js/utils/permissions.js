...
export function canAccessRoute(route, session) {
  ...
  if (route.view === 'dashboard' || route.view === 'projects' || route.view === 'toolkit' || route.view === 'kanban' || route.view === 'placeholder') {
  ...
  if (route.view === 'project') {
    ...
    if (['finance'].includes(route.params?.tab) && !canViewFinancials(session)) return false;
    return true;
  }
