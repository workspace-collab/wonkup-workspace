...
export const AppsScriptAdapter = {
  exchangeCode(code) {
    return post('auth.exchangeCode', { code });
  },

  validateSession(session) {
    return post('auth.validate', { sessionToken: session?.token });
  },

  revokeSession(session) {
    return post('auth.revoke', { sessionToken: session?.token });
  },

  listWorkspaces(session) {
    return post('workspaces.list', { sessionToken: session?.token });
  }
};
